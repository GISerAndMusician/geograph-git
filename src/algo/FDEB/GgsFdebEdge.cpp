...
GgsFdebEdge::GgsFdebEdge(std::string sourceLabel_, std::string targetLabel_,
...
std::vector<meerkat::mk_vector2> GgsFdebEdge::points()
{
	std::vector<meerkat::mk_vector2> points = _subdivs;
	points.insert(points.begin(), _start);
	points.push_back(_end);

	return points;
}

meerkat::mk_vector2 GgsFdebEdge::center(meerkat::mk_vector2 &p1_, meerkat::mk_vector2 &p2_)
{
    return (p1_+p2_)/2.0;
}

meerkat::mk_vector2 GgsFdebEdge::project(meerkat::mk_vector2 &point_,
...
meerkat::mk_vector2 GgsFdebEdge::vector()
{
    return _end-_start;
}

void GgsFdebEdge::add_subdivisions()
{
    ...
    else
    {
        ...
        while( subdivIndex < newSubdivsNum )
        {
            ...
            subdivIndex++;
            if( r + segmentLength - 1.0 > 1e-8 ) // if( r + segmentLength > 1.0 )
            {
...
void GgsFdebEdge::arrange_direction()
{
...
void GgsFdebEdge::add_spring_forces(std::vector<meerkat::mk_vector2> &forces_, double K_)
{
...
void GgsFdebEdge::add_electrostatic_forces(std::vector<meerkat::mk_vector2> &forces_,
                                    GgsFdebEdge edge_, double epsilon_)
{
    int len = (int)_subdivs.size();
...
void GgsFdebEdge::add_gravitational_forces(std::vector<meerkat::mk_vector2> &forces_,
...
void GgsFdebEdge::update(std::vector<meerkat::mk_vector2> &forces_, double S_)
{
    int len = (int)_subdivs.size();
    double flen = 0.0;
    for( int i=0; i<len; i++ )
...
void GgsFdebEdge::smooth(double sigma_)
{
    int len = (int)_subdivs.size();
...
double GgsFdebEdge::edge_visibility(GgsFdebEdge &edge1_, GgsFdebEdge &edge2_)
{
...
double GgsFdebEdge::angle_compatilibity(GgsFdebEdge &edge1_, GgsFdebEdge &edge2_)
{
...
double GgsFdebEdge::scale_compatibility(GgsFdebEdge &edge1_, GgsFdebEdge &edge2_)
{
...
double GgsFdebEdge::position_compatibility(GgsFdebEdge &edge1_, GgsFdebEdge &edge2_)
{
...
double GgsFdebEdge::visibility_compability(GgsFdebEdge &edge1_, GgsFdebEdge &edge2_)
{
    return std::min(edge_visibility(edge1_, edge2_),
                    edge_visibility(edge2_, edge1_));
}

int compare_edges( GgsFdebEdge edge1_,  GgsFdebEdge edge2_ )
{
