...
private:
    // Network structure
    std::map<std::string, GgsFdebNode> _nodes;
    std::vector<GgsFdebEdge> _edges;
...
public:
	...
	std::map<std::string, GgsFdebNode> nodes() { return _nodes; }

	std::vector<GgsFdebEdge> edges() { return _edges; }
     ...
     */
    void load(std::map<std::string, GgsFdebNode> nodes, std::vector<GgsFdebEdge> edges);
